...
| detached | Off the customer at Stripe, card fields cleared locally. Terminal for this payment method |
...
| detached | on file | Not for this payment method. A new card goes through create with a new id |
...
- Detach is permanent. Re-adding the same card produces a new payment method with a new id.
...
| ---- | ----- | ----------------- |
| Delete while the subscription is active | Endpoint called directly, or the client failed to hide the control | Refuse API side. Pulling the card only books a failed renewal |
...
| No card on file | Already removed, or never had one | Return success, nothing to detach |
...
| Detach succeeds, local clear fails | Partial failure | Card is gone at Stripe and still displays. Nothing renews, so no billing impact. User retries and the already detached path clears the row, or a manual sync (admin |
...
| `payment_method.detached` for an out of band removal | Card pulled from the Dashboard | Same handler clears the local row |
| User subscribes again later | Card was removed | Create flow collects a card from scratch. The old id is not reusable |

## Decisions

Gate on chargeability, not on subscription status alone. Cancelled with a grace period and ended both mean no invoice is coming, and both allow removal. Keying on `active` would lock a cancelled user out of removing their card for the rest of the paid term.
...
- Decide whether a confirmation prompt guards the delete. It is permanent and re-adding means a fresh card through create.
- Pin the exact subscription states that pass the gate, in one place both the client and the API read.
- Decide the behavior for a user with a card on file and no subscription row at all.
...
- Replacing the card. See the update flow.
- Cancel and resume.
