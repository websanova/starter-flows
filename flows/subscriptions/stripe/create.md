...
| Term | Description |
| --- | --- |
| API | The back end. Holds the API records and talks to Stripe. |
...
| App Storage | Short lived storage in the App that survives a redirect away and back. Holds the Stripe Checkout Session secret across a confirm. |
| Auth User | The signed in User's data held by the App. Refreshed whenever subscription state changes. |
| Stripe Billing Address Element | The Stripe Element collecting the billing address and name. |
| Stripe Checkout Session | The Stripe object this flow hangs off. Carries the line items, the address, the promotion code, the total and the Stripe Payment Method. |
| Stripe Customer | The Stripe object holding the User's id, address and saved Stripe Payment Methods. |
| Stripe Element | A Stripe UI component mounted by the App. Two are used here. |
...
| Stripe Subscription | The subscription at Stripe. Created inside the confirm, never before. |
| User | The human subscribing. Never the App and never the API. |
