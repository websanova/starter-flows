...
The trial branch is what ruled this strategy out. A subscription created with `trial_period_days` lands at `trialing`, not `incomplete`, because the first invoice is `$0` and nothing is owed. Stripe stamps `trial_end` on that same call, so the trial clock starts when the user lands on the page rather than when the user subscribes. Land on Monday, pay Thursday, and three of the fourteen days are already gone. Setting `payment_behavior` to `default_incomplete` does not hold it back, because that only applies when the first invoice requires payment. There is no Stripe setting that keeps a trial subscription out of `trialing` until a SetupIntent is confirmed. The 23 hour expiry that cleans up abandoned `incomplete` subscriptions does not apply to a `trialing` one either, so an abandoned trial runs its full length against a customer with no stored card. The only way to start the clock at subscribe is to not create the subscription until the card exists, which is [create.md](../create.md).
